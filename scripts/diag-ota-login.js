 ...
 *   OTA_PROXY=http://user:pass@ip:port node scripts/diag-ota-login.js   # thử qua proxy
 *
...
// Cùng cách parse như server.js để test đúng proxy sẽ dùng khi chạy thật.
const otaProxyUrl = process.env.OTA_PROXY || "";
const otaHost = new URL(baseUrl).host;
let otaProxy = null;
if (otaProxyUrl) {
  const u = new URL(otaProxyUrl);
  otaProxy = {
  ...
  console.log(`Proxy: ${otaProxy.protocol}://${otaProxy.host}:${otaProxy.port} (chỉ cho ${otaHost})`);
}
...
async function probe(name, config) {
  try {
    const useProxy =
      otaProxy && new URL(config.url, baseUrl).host === otaHost;
    const res = await axios({
      validateStatus: () => true,
      maxRedirects: 0,
      timeout: 30000,
      ...config,
      ...(useProxy ? { proxy: otaProxy } : {}),
    });
