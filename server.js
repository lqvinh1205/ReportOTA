    ...
    try {
        ...
        for (const searchType of searchTypes) {
            ...
            do {
                ...
                console.log(`🔗 Fetching ${searchType.name} page ${currentPage} with params:`, pageParams);
                ...
                if (parsedData.success) {
                    ...
                    console.log(`✅ ${searchType.name} - Page ${currentPage}/${totalPages} - Found ${parsedData.bookingsOnPage} bookings`);
                    
                    // If only 1 page, break early
                    if (totalPages === 1) {
                        console.log(`📄 ${searchType.name} - Only 1 page of data available`);
                        break;
                    }
                } else {
                    console.log(`❌ ${searchType.name} - Failed to parse page ${currentPage}`);
                    break;
                }
                ...
                if (fetchedPages >= 20) {
                    console.log(`⚠️  ${searchType.name} - Reached safety limit of 20 pages`);
                    break;
                }
            ...
            console.log(`🎉 ${searchType.name} - Completed! ${typeBookings.length} bookings from ${fetchedPages} pages`);
        ...
        console.log(`📑 Total pages processed: ${fetchSummary.totalPagesProcessed}/${fetchSummary.totalPages}`);
        fetchSummary.searchTypes.forEach(type => {
            console.log(`   ${type.name}: ${type.bookings} bookings (${type.pages}/${type.totalPages} pages)`);
        });
...
function parseBookingData(html) {
    try {
        ...
        const paginateBlockMatch = html.match(/<div[^>]*class="[^"]*dataTables_paginate[^"]*"[^>]*>([\s\S]*?)<\/div>/i);
        if (paginateBlockMatch) {
            const paginateHtml = paginateBlockMatch[1];

            // Find all page number links
            const pageNumberMatches = [...paginateHtml.matchAll(/<a[^>]*class="[^"]*paginate_button(?!.*current)[^"]*"[^>]*href="[^"]*p=(\d+)[^"]*"[^>]*>(\d+)<\/a>/gi)];
            const currentPageMatch = paginateHtml.match(/<a[^>]*class="[^"]*paginate_button current[^"]*"[^>]*href="[^"]*p=(\d+)[^"]*"[^>]*>(\d+)<\/a>/i);
            ...
            console.log('📄 Pagination found (DataTables):', currentPage, '/', totalPages);
        } else {
            // Fallback: Try to match old pagination text
            const paginationMatch = html.match(/Trang\s+(\d+)\s+\/\s+(\d+)|Page\s+(\d+)\s+of\s+(\d+)/i);
            if (paginationMatch) {
            currentPage = parseInt(paginationMatch[1] || paginationMatch[3]) || 1;
            totalPages = parseInt(paginationMatch[2] || paginationMatch[4]) || 1;
            console.log('📄 Pagination found (text):', currentPage, '/', totalPages);
            }
        }
        
        // Find the main booking table
        const bookingTableMatch = html.match(/<table[^>]*class="[^"]*table[^"]*"[^>]*>([\s\S]*?)<\/table>/i);
        ...
        if (bookingTableMatch) {
            ...
            for (let i = 1; i < rowMatches.length; i++) { // Skip header row
                ...
                if (cells.length >= 15) { // Ensure we have enough columns
                    const booking = {
                    ...
                    // Clean up the data
                    booking.totalAmount = booking.totalAmount.replace(/[^\d,.-]/g, '').trim();
